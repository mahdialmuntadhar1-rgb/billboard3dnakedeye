...
var requireAuth = /* @__PURE__ */ __name(async (c, next) => {
  try {
    ...
    const decoded = (0, import_jsonwebtoken.verify)(token, JWT_SECRET);
    c.user = decoded;
    await next();
  } catch (error3) {
...
var D1Client = class {
  ...
  async getContacts(params) {
    ...
    const contacts2 = (contactsResult.results || []).map((row) => ({
      ...row,
      tags: this.parseJSON(row.tags)
    }));
    return { contacts: contacts2, total, pages };
  }
  async getContactById(id) {
    ...
    return {
      ...result,
      tags: this.parseJSON(result.tags)
    };
  }
  ...
  async getImportJobById(id) {
    const result = await this.db.prepare("SELECT * FROM import_jobs WHERE id = ?").bind(id).first();
    if (!result) return null;
    return {
      ...result,
      error_details: this.parseJSON(result.error_details)
    };
  }
  async getImportJobsByUserId(userId) {
    const results = await this.db.prepare(
      "SELECT * FROM import_jobs WHERE user_id = ? ORDER BY created_at DESC"
    ).bind(userId).all();
    return (results.results || []).map((row) => ({
      ...row,
      error_details: this.parseJSON(row.error_details)
    }));
  }
};

// src/utils/phone-normalization.ts
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
function detectLanguage(governorate) {
  const soraniGovernorates = ["Sulaymaniyah", "Halabja", "Erbil", "Kirkuk"];
  const bahdiniGovernorates = ["Duhok", "Zakho"];
...
contacts.get("/", requireAuth, async (c) => {
  ...
  const result = await db.getContacts({
    ...
    governorates: governorates.length > 0 ? governorates : void 0
  });
  return c.json({
    success: true,
...
contacts.put("/:id", requireAuth, async (c) => {
  ...
  if (body.governorate !== void 0) updateData.governorate = body.governorate;
  if (body.language !== void 0) updateData.language = body.language;
...
contacts.delete("/bulk", requireAuth, async (c) => {
  const db = new D1Client(c.env.DB);
...
contacts.get("/governorates/counts", requireAuth, async (c) => {
  const db = new D1Client(c.env.DB);
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var campaigns = new Hono2();
campaigns.get("/", requireAuth, async (c) => {
  const db = new D1Client(c.env.DB);
  const page = parseInt(c.req.query("page") || "1");
  const limit = parseInt(c.req.query("limit") || "50");
...
campaigns.put("/:id", requireAuth, async (c) => {
  const db = new D1Client(c.env.DB);
  const id = c.req.param("id");
  const body = await c.req.json();
  const updateData = {};
...
campaigns.delete("/:id", requireAuth, async (c) => {
  const db = new D1Client(c.env.DB);
  const id = c.req.param("id");
...
campaigns.post("/:id/send", requireAuth, async (c) => {
  const db = new D1Client(c.env.DB);
  const id = c.req.param("id");
...
uploads.post("/presigned-url", requireAuth, async (c) => {
...
uploads.post("/process", requireAuth, async (c) => {
  const db = new D1Client(c.env.DB);
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      const existing = await db.getContacts({
        ...
        governorates: row.governorate ? [row.governorate] : void 0
      });
      if (existing.contacts.length > 0) {
        ...
        await db.createContact({
          ...
          governorate: row.governorate,
          email: row.email,
          tags: row.tags ? row.tags.split(";") : void 0
        });
        inserted++;
      }
    } catch (error3) {
      errors++;
      errorDetails.push({ row: i + 2, message: error3.message });
    }
  }
...
auth.post("/nabda/login", async (c) => {
  ...
  try {
    ...
    console.error("Nabda login error:", error3);
...
auth.get("/nabda/instance", async (c) => {
  ...
  try {
    ...
    console.error("Get instance error:", error3);
...
auth.post("/nabda/select-instance", async (c) => {
  ...
  try {
    ...
    console.error("Select instance error:", error3);
...
auth.post("/logout", async (c) => {
  ...
  try {
    ...
    const user = await db.getUserById(decoded.id);
    if (user) {
      await db.updateUser(user.id, { nabda_session_token: void 0 });
    }
...
var JobStatus = class extends DurableObject {
  static {
    __name(this, "JobStatus");
  }
  state;
...
app.route("/api/campaigns", campaigns_default);
