...
var D1Client = class {
  ...
  async getMessageLogById(id) {
    const result = await this.db.prepare("SELECT * FROM message_logs WHERE id = ?").bind(id).first();
    return result;
  }
  async updateMessageLogStatus(id, status, data) {
    const updates = [];
    const values = [];
    updates.push("status = ?");
    values.push(status);
    if (data?.nabda_message_id !== void 0) {
  ...
  async createShakuUser(data) {
    ...
    await this.db.prepare(
      ...
      data.photo_url || null,
      data.role || "user",
      data.onboarded !== void 0 ? data.onboarded : 0,
...
campaigns.post("/:id/send", async (c) => {
  ...
  let contactsResult = await db.getContacts({ page: 1, limit: 1e3 });
  let contacts2 = contactsResult.contacts;
  if (contactIds && Array.isArray(contactIds) && contactIds.length > 0) {
    contacts2 = contacts2.filter((c2) => contactIds.includes(c2.id));
  }
  if (dryRun) {
    return c.json({
      success: true,
      ...
      wouldSendTo: contacts2.length,
      sampleContacts: contacts2.slice(0, 5).map((c2) => ({ name: c2.name, phone: c2.phone }))
    });
  }
  ...
  await db.updateCampaign(id, {
    status: "queued",
    total_recipients: contacts2.length,
    pending_count: contacts2.length,
    sent_count: 0,
    failed_count: 0
  });
  let queuedCount = 0;
...
campaigns.post("/:id/process", async (c) => {
  ...
  let sent = 0;
  let failed = 0;
  for (const log4 of queuedLogs) {
    try {
      await db.updateMessageLogStatus(log4.id, "sending");
      ...
      if (response.ok) {
        sent++;
        await db.updateMessageLogStatus(log4.id, "sent", {
        ...
        await db.updateMessageLogStatus(log4.id, "failed", { error: String(err) });
      }
    } catch (error3) {
      failed++;
      await db.updateMessageLogStatus(log4.id, "failed", { error: error3.message });
    }
    if (queuedLogs.indexOf(log4) < queuedLogs.length - 1) {
  ...
  await db.updateCampaign(id, {
    ...
    ...hasMore ? {} : { completed_at: (/* @__PURE__ */ new Date()).toISOString() }
  });
  return c.json({
    success: true,
...
nabda.post("/send-test", async (c) => {
  ...
  try {
    const db = new D1Client(c.env.DB);
    const log4 = await db.createMessageLog({
      recipient: normalizedPhone,
      message,
      status: "pending"
    });
    ...
    if (!response.ok) {
      await db.updateMessageLogStatus(log4.id, "failed", {
    ...
    console.error("Send test message error:", error3);
...
nabda.post("/otp", async (c) => {
  ...
  try {
    ...
    console.error("Send OTP error:", error3);
    return c.json({ error: error3.message || "Failed to send OTP" }, 500);
  }
});
nabda.get("/status", async (c) => {
  ...
  try {
    ...
    console.error("Get status error:", error3);
    return c.json({ error: error3.message || "Failed to get status" }, 500);
  }
});
...
nabda.get("/balance", async (c) => {
  ...
  try {
    ...
    return c.json({ error: error3.message || "Failed to get balance" }, 500);
  }
});
nabda.get("/templates", async (c) => {
  ...
  try {
    ...
    console.error("Get templates error:", error3);
    return c.json({ error: error3.message || "Failed to get templates" }, 500);
  }
});
var nabda_default = nabda;

// src/routes/message-logs.ts
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var messageLogs = new Hono2();
...
messageLogs.get("/:id", async (c) => {
  const db = new D1Client(c.env.DB);
  const id = c.req.param("id");
  const log4 = await db.getMessageLogById(id);
  if (!log4) {
    return c.json({ error: "Message log not found" }, 404);
  }
  return c.json({ success: true, log: log4 });
});
messageLogs.put("/:id/status", async (c) => {
  const db = new D1Client(c.env.DB);
  const id = c.req.param("id");
  const { status, nabda_message_id, error: error3, delivered_at } = await c.req.json();
  if (!status) {
    return c.json({ error: "Status is required" }, 400);
  }
  const log4 = await db.updateMessageLogStatus(id, status, {
    nabda_message_id,
    error: error3,
    delivered_at
  });
...
var message_logs_default = messageLogs;

// src/routes/worker.ts
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var worker = new Hono2();
worker.post("/process", async (c) => {
  ...
  const campaignsResult = await db.getCampaigns({ page: 1, limit: 1e3 });
  const activeCampaigns = campaignsResult.campaigns.filter(
    (c2) => c2.status === "queued" || c2.status === "sending"
  );
...
// src/routes/shaku-auth.ts
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var shakuAuth = new Hono2();
...
async function signJWT(payload, secret) {
  ...
  const b64Payload = btoa(JSON.stringify({ ...payload, iat: Math.floor(Date.now() / 1e3) })).replace(/=/g, "");
...
shakuAuth.post("/reset-confirm", async (c) => {
  ...
  if (!reset || reset.used || new Date(reset.expires_at) < /* @__PURE__ */ new Date()) {
...
var shaku_auth_default = shakuAuth;
...
app.route("/api/templates", templates_default);
app.route("/api/uploads", uploads_default);
app.route("/api/auth", auth_default);
app.route("/api/nabda", nabda_default);
app.route("/api/message-logs", message_logs_default);
app.route("/api/worker", worker_default);
app.route("/api/shaku-auth", shaku_auth_default);
app.notFound((c) => {
  return c.json({
    error: "Not Found",
